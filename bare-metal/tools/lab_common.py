...
# -- Iterating across a bunch of machines to do the same thing ---
# --- The Class-Based (Reasonable?) Implementation             ---
...
class TaskRunner:
   ...
   def __init__(self, machines, connection_args, the_task_class, task_arg=None):
   ...
   def run(self):
      ...
      # Perform pre-launch pass, intnedned to get every machine into whatever
      # pre-task-submit state is required,  for examplepowering them off first.

      self.the_task_class.announce_pre_launch_pass()
      for machine in list(self.tasks.keys()):
         task = self.tasks[machine]
         if not task.pre_launch():
            blurt("Abandoning futher action for %s due to preceeding errors." % machine)
            del self.tasks[machine]
      #
      if not self.tasks:
         blurt("No machines successfully estbalished pre-launch conditions.")
         return
         ...
         for machine in list(pending_tasks.keys()):
            ...
            try:
               ...
               if task_has_ended(task_res):
                  ...
                  task.ending_task_ress = task_res ##
               else:
                  task_state = task_res["TaskState"]
                  if task_state == "Starting":
...
class RunnableTask:
   ...
   @classmethod
   def announce_pre_launch_pass(self):
      return
