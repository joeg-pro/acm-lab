...
resource junos_interface_physical sw_xe_1_port {
  ...
  for_each = local.switch_port_configs["sw_xe_1"]
    name         = each.key
    description  = each.value.description
...
resource junos_vlan sw_xe_2_vlan {

  provider = junos.sw_xe_2
...
resource junos_interface_physical sw_xe_2_port {

  depends_on = [junos_vlan.sw_xe_2_vlan]

  provider = junos.sw_xe_2

  for_each = local.switch_port_configs["sw_xe_2"]
    name         = each.key
    description  = each.value.description
    trunk        = length(each.value.vlans) > 1
    vlan_members = each.value.vlans
